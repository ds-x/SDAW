...
// box - exact with Vec3 Radius
float sdfBox2(vec3 pos, vec3 center, vec3 radius) {
...
//Link - exact
float sdfLinkX( vec3 p, float le, float r1, float r2 )
{
...
float sdfCylinderInf( vec3 p, vec3 c )
{
  return length(p.xz-c.xy)-c.z;
}

//Cone - exact
float sdfCone( in vec3 p, in vec2 c, float h )
{
...
float sdfConeB( vec3 p, vec2 c, float h )
{
  float q = length(p.xz);
...
//Infinite Cone - exact
float sdfConeInf( vec3 p, vec2 c )
{
...
//Capped Cylinder - exact
float sdfCappedCylinder2(vec3 p, vec3 a, vec3 b, float r)
{
  vec3  ba = b - a;
  vec3  pa = p - a;
...
//Capped Cone - exact
float sdfCappedCone( vec3 p, float h, float r1, float r2 )
{
  vec2 q = vec2( length(p.xz), p.y );
  vec2 k1 = vec2(r2,h);
...
//Capped Cone - exact
float sdfCappedCone2(vec3 p, vec3 a, vec3 b, float ra, float rb)
{
    float rba  = rb-ra;
...
//Solid Angle - exact
float sdfSolidAngle(vec3 p, vec2 c, float ra)
{
  // c is the sin/cos of the angle
  vec2 q = vec2( length(p.xz), p.y );
  float l = length(q) - ra;
...
//Round Cone - exact
float sdfRoundCone2(vec3 p, vec3 a, vec3 b, float r1, float r2)
{
...
//Rhombus - exact
float sdfRhombus(vec3 p, float la, float lb, float h, float ra)
{
  p = abs(p);
  vec2 b = vec2(la,lb);
...
float sdfOctahedronB( vec3 p, float s)
{
  p = abs(p);
